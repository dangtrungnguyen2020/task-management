import { jest, describe, expect, beforeAll, it } from '@jest/globals';
...
describe('Auth Controller', () => {
  ...
  const mockAuthService: Partial<AuthService> = {
    login: jest.fn(async (payLoad): Promise<ResLoginDto> => {
      return mockLoginResponse;
    }),
    logoutFromAll: jest.fn<AuthService['logoutFromAll']>(),
  ...
  describe('Login', () => {
    it('should raise exception when passing login and password is empty', async () => {
      // Arrange
      const credentials: ReqLoginDto = {
        email: '',
        password: '',
      };
      ...
      try {
        // Act
        await controller.login(mockRequest, credentials);
      } catch (error) {
        // Assert
